CLI Log
-------
Michael_Krokos: export EXP_PATH=`/workspace/scripts/gen_dir.sh /workspace/experiments/language_modeling` && PYTHONPATH=/workspace/language_modeling/src \
...
--data.path /workspace/data/pile/sage --optimizer=AdamW --optimizer.lr 3e-5 --lr_scheduler=CosineWarmupScheduler --lr_scheduler.warmup 30000 \
--lr_scheduler.total_steps 215332 --model.lr_scheduler_interval step --trainer.callbacks LearningRateMonitor --trainer.callbacks ModelCheckpoint \
...
--trainer.gradient_clip_val 1.0 --trainer.accumulate_grad_batches 16 &
