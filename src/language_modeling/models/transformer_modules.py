...
class PositionalEncoding(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        positions = einops.rearrange(
            torch.arange(self.seq_length), "seq_length -> seq_length 1"
        )
        ...
        pe = einops.rearrange(pe, "seq_length d_model -> 1 seq_length d_model")
...
class MultiHeadAttention(nn.Module):
    ...
    def _apply_mask(self, attention_logits, padding_mask, causal_mask):
        ...
        if padding_mask is not None:
            padding_mask = einops.rearrange(
                padding_mask, "batch seq_len -> batch 1 1 seq_len"
            )
            attention_logits = attention_logits.masked_fill(
                padding_mask == 0, large_neg_value
            )

        if causal_mask is not None:
            attention_logits = attention_logits.masked_fill(
                causal_mask == 0, large_neg_value
            )

        return attention_logits

    def forward(self, query, value=None, padding_mask=None, causal_mask=None):
        ...
        attention_logits = self._apply_mask(
            attention_logits, padding_mask=padding_mask, causal_mask=causal_mask
        )
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, causal_mask, mask):
        attended_x, _ = self.multi_head_attention(
            x, causal_mask=causal_mask, padding_mask=mask
        )
...
class Encoder(nn.Module):
    def __init__(
        self, num_layers, num_heads, d_model, d_ff, dropout, seq_len, causal_mask=False
    ):
